...
public class GheDAO extends BaseDAO {
    ...
    public int countByMayBayAndHangGhe(int mayBayId, int hangGheId) {
        String sql = "select count(*) from ghe where maybay_id=? and hangghe_id=?";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, mayBayId);
            ...
            throw new RuntimeException("ghe countByMayBayAndHangGhe failed", e);
        }
    }

    public boolean existsByMayBay(int mayBayId) {
        String sql = "select 1 from ghe where maybay_id=? limit 1";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, mayBayId);
            try (ResultSet rs = ps.executeQuery()) {
            ...
            throw new RuntimeException("ghe existsByMayBay failed", e);
        }
    }

    public void deleteByMayBay(int mayBayId) {
        String sql = "delete from ghe where maybay_id=?";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, mayBayId);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException("ghe deleteByMayBay failed", e);
        }
    }

    public boolean existsUsedInVeByMayBay(int mayBayId) {
        String sql =
                "select 1 " +
                "from ve v " +
        ...
        try (Connection c = getConnection();
            ...
            throw new RuntimeException("ghe existsUsedInVeByMayBay failed", e);
        }
    }
}