...
public class GiaGheOverrideDAO extends BaseDAO {
    ...
    public BigDecimal findPrice(int chuyenBayId, int gheId) {
        ...
        try (Connection c = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return rs.getBigDecimal("giaoverride");
            }
    ...
    public void deleteByMayBay(int mayBayId) {
        String sql =
                "delete from giagheoverride " +
                "where ghe_id in (select ghe_id from ghe where maybay_id=?)";
        ...
        try (Connection c = getConnection();
            ...
            throw new RuntimeException("giagheoverride deleteByMayBay failed", e);
        }
    }
}